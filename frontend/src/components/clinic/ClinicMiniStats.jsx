 ...
 */
export default function ClinicMiniStats({ clinic, className = '', hideDoctorCount = false, variant = 'card' }) {
  ...
  return (
    <div className={`${listClass} ${className}`}>
      {chips.map((c) => (
        <span
          key={c.key}
          className={`${variant === 'modal' ? '' : 'shrink-0 snap-start '}inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold border ${tones[c.tone]}`}
        >
