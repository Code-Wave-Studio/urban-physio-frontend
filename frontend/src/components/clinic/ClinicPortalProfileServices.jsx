...
/** Treatment service cards for the clinic public profile — Clinic Portal. */
export default function ClinicPortalProfileServices({ clinicId }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await clinicPortal.profileServices(clinicId);
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await clinicPortal.updateProfileService(clinicId, editingId, payload);
      } else {
        await clinicPortal.createProfileService(clinicId, payload);
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await clinicPortal.deleteProfileService(clinicId, id);
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs text-slate-500">
        Treatment cards shown on your public clinic profile (name, price, description). Separate from the tag lists above.
      </p>
