...
export default function ClinicCreatePackagePage() {
  ...
  return (
    ...
    >
      <div className="grid lg:grid-cols-[minmax(0,1fr)_minmax(340px,0.7fr)] gap-5">
        <section className="glass-card !p-0 overflow-hidden">
          <div className="px-5 py-4 border-b"><h2 className="font-bold">Package catalog</h2></div>
            ...
            <div className="divide-y divide-slate-100">
              {rows.map((row) => (
                <div key={row.id} className="p-4 flex flex-col sm:flex-row sm:items-center gap-3">
                  <div className="flex-1"><div className="flex items-center gap-2"><p className="font-semibold text-slate-900">{row.name}</p><span className={`text-[10px] px-2 py-0.5 rounded-full ${Number(row.is_active) ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>{Number(row.is_active) ? 'Active' : 'Inactive'}</span></div><p className="text-xs text-slate-500 mt-1">{row.total_sessions} sessions · {row.duration_days} days · ₹{Number(row.price || 0).toLocaleString('en-IN')}</p><p className="text-xs text-slate-400 mt-1">{row.description}</p></div>
                  <div className="flex gap-2"><button type="button" className="btn-outline text-xs !py-1.5" onClick={() => edit(row)}><FaIcon icon="fa-pen" className="mr-1" />Edit</button><button type="button" className="text-xs font-semibold text-rose-600 px-2" onClick={() => remove(row)}>Delete</button></div>
                </div>
              ))}
        ...
        <form onSubmit={save} className="glass-card !p-5 space-y-4 self-start lg:sticky lg:top-4">
          <div className="flex justify-between"><h2 className="font-bold">{editing ? 'Edit package' : 'New package'}</h2>{editing && <button type="button" className="text-xs text-slate-500" onClick={() => { setEditing(null); setForm(initial); }}>Cancel edit</button>}</div>
