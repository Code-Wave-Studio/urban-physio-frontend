...
export default function PrescriptionDocumentViewPage() {
  ...
  useEffect(() => {
    try {
      const storageKey = `tup_prescriptions_${patientKey}`;
      const raw = localStorage.getItem(storageKey);
      if (raw) {
        const list = JSON.parse(raw);
        const match = list.find((item) => item.rx_number === rxNum || item.id === rxNum);
        if (match) {
          setRx(match);
          setLoading(false);
          return;
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4">
        <div className="flex items-center gap-2 text-slate-600 font-semibold text-sm">
          <FaIcon icon="fa-circle-notch" className="animate-spin text-teal-600" />
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-800 antialiased p-2 sm:p-6">
      {/* Top Floating Control Bar */}
      <div className="no-print max-w-4xl mx-auto mb-4 bg-white/90 backdrop-blur-md p-3 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between gap-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h1 className="font-bold text-sm text-slate-900">Medical Prescription {rx.rx_number}</h1>
            <p className="text-[11px] text-slate-500">Official Clinical A4 Document &bull; {patientKey}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={handlePrint}
            className="btn-primary text-xs px-4 py-2 inline-flex items-center gap-1.5 shadow-sm font-bold"
          >
      ...
      {/* A4 Printable Prescription Document Sheet */}
      <div id="urban-physio-print-root" className="max-w-4xl mx-auto bg-white p-6 sm:p-10 rounded-2xl border border-slate-200 shadow-md space-y-6">
        ...
        <div className="border-b-2 border-teal-600 pb-5 flex flex-wrap justify-between items-start gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2.5 text-teal-800 font-extrabold text-xl tracking-tight">
              <span className="w-10 h-10 rounded-xl bg-teal-600 text-white flex items-center justify-center text-base font-bold shadow-xs">
                UP
              </span>
          ...
          <div className="text-right text-xs space-y-0.5">
            <p className="font-extrabold text-sm text-slate-900">{rx.doctor_name}</p>
            <p className="text-teal-700 font-medium">{rx.doctor_qualification}</p>
            <p className="text-slate-500 font-mono">Reg No: {rx.doctor_reg_no}</p>
          </div>
        </div>
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
          <div>
            ...
            <p className="font-bold text-slate-900 text-sm mt-0.5">{patientKey}</p>
          </div>
          <div>
            <p className="text-slate-400 font-semibold uppercase text-[10px]">Prescription No.</p>
            <p className="font-bold text-teal-800 font-mono mt-0.5">{rx.rx_number}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <span className="inline-block mt-0.5 text-[10px] uppercase font-bold bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded-full">
              Verified
            </span>
          </div>
        </div>
        ...
        <div className="pt-8 flex justify-between items-end text-xs border-t border-slate-200 mt-8">
          <div className="text-[10px] text-slate-500 space-y-1 max-w-[280px]">
            ...
            <p className="text-slate-400 font-mono">Prescription ID: {rx.rx_number} &bull; v{rx.version || 1}.0</p>
          </div>
          <div className="text-right space-y-1.5">
            ...
            <p className="font-extrabold text-sm text-slate-900">{rx.doctor_name}</p>
            <p className="text-xs font-semibold text-teal-800">{rx.doctor_qualification}</p>
            <p className="text-xs text-slate-500 font-mono">Reg. No: {rx.doctor_reg_no}</p>
          </div>
        </div>
      </div>
