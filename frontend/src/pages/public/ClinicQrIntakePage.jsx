...
export default function ClinicQrIntakePage() {
  ...
  const submit = async (event) => {
    ...
    setSubmitting(true);
    try {
      const payload = { token, ...values, ...assessmentPayload() };
    ...
    <main className="min-h-screen bg-gradient-to-b from-teal-50 via-slate-50 to-white px-4 py-8">
      <div className="max-w-xl mx-auto bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        <header className="bg-teal-700 text-white p-6">
          ...
          <p className="text-sm text-teal-50 mt-2">New patients register once. Returning patients sign in with OTP — no password needed.</p>
        </header>
          ...
          {mode === 'returning' ? (
            otpStep === 'phone' ? (
              <form onSubmit={sendReturningOtp} className="space-y-4">
                ...
                <p className="text-[11px] text-center text-slate-400">We’ll text / WhatsApp / email a 6-digit code — passwordless.</p>
              </form>
            ) : (
